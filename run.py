...
parser.add_argument("method",choices=['stologdrs','detlogdrs','gdbaseline'])
parser.add_argument("--penalty",type=float,default=128.0,help="penalty coefficient of the ADMM solver")
parser.add_argument("--maxiter",type=int,default=150000,help="maximum iteration before termination")
...
parser.add_argument("--ss_init",type=float,default=8e-3,help="step size initialization")
parser.add_argument("--ss_scale",type=float,default=0.25,help="step size scaling")
parser.add_argument("--output_dir",type=str,default="unsu_run_saved",help="output filename")
parser.add_argument("--seed",type=int,default=None,help="random seed for reproduction")
parser.add_argument("--dataset",type=str,default="condindp2v",help="specify the dataset to use")
...
nz_set = np.array([len(data_dict['py'])]) # FIXME: assume knowing the cardinality of Z
...
repeat_cnt = 0
safe_savename_base = "run_{:}_{:}_{:}".format(args.method,args.dataset,timenow.strftime("%Y%m%d"))
